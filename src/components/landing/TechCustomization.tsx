          ...
          {techStack.map((item, i) => (
            ...
            >
              <div className="absolute -top-6 left-1/2 -translate-x-1/2 min-w-[230px] px-8 py-3 rounded-[22px] bg-[#ededed] border border-white/90">
                <h3 className="text-[24px] font-semibold text-center text-foreground leading-none whitespace-nowrap">
                  {item.category}
                </h3>
              </div>
              ...
              <div className="flex items-center justify-between gap-4 md:gap-6 flex-nowrap whitespace-nowrap py-1">
                {item.technologies.map((tech) => (
                  <img
                    ...
                    alt={tech.name}
                    className={`${
                      tech.name === "Elasticsearch"
                        ? "h-11 md:h-14 max-w-[130px] md:max-w-[160px]"
                        : "h-10 md:h-12 max-w-[110px] md:max-w-[140px]"
